...
export async function GET(request: Request) {
  try {
    ...
    const rows = await reportService.getPaymentsHistory({ 
      from, 
      to,
      professionalId,
      patientId,
    ...
    console.error("[GET /api/admin/reports/payments] Error", error);
    const message =
      error instanceof Error ? error.message : "Error interno al obtener el historial de pagos.";
